var cursorPos, Paste, createHiddenEditable, isFocusable, initialLine, initialPosition, targetElement, itemId, dataManger, elementNode, getAllCommentAjax;

var dashboardTypeId = "#sync_report_viewer";
...
$(document).ready(function () {
    ...
    $(document).on('click', '#post-comment', function () {
        ...
        if (itemType.toLowerCase() == 'report') {
        ...
        var comment = postmd.value();
        var commentHTML = postmd.options.previewRender(comment).replace("#", "");
        comment = getRelativePath(comment);
        var commentOriginalValue = comment;
        ...
        if (comment.length > 4000) {
            ...
            $(".comment-area").val("");
            $(".error-message").html("");
            var values = { itemId: itemId, comment: comment, itemType: itemType, dashboardItemId: dashboardItemId, url: decodeURI(currentPageUrl), uniqueUsersValue: userMention.uniqueUsersValue, uniqueGroupValue: userMention.uniqueGroupValue, mentionedAll: userMention.mentionedAll, parentDashboardId: parentDashboardId, CommentAction: commentAction.Add, currentDate: date  };
            showWaitingPopup("parent-scroll-element");
            parent.ajaxPostCall("POST", addCommentUrl, values, null, OnCommentPosted, null, null);
        }
    });
    ...
    function OnCommentPosted(data) {
        if (data) {
            var addComment = "<li id='comment-"
                ...
                + data.Comments[0].UserId + "'><img src='" + avatarUrl + "?id="
                + data.Comments[0].IdPReferenceId + "&ImageSize=32' class='avatar'> </div></div><div class='post-body'> <header><span class='post-byline col-xs-5 no-padding'><span title='"
                + data.Comments[0].UserDisplayName + "'class='author publisher-anchor-color'>"
                + data.Comments[0].UserDisplayName + "</span><span class='su su-status-active'></span></span><menu><li class='reply' title='" + window.Server.App.LocalizationContent.ReplyButton + "' data-toggle='tooltip' data-placement='bottom'> <a class='reply-comment'><span class='su su-reply'></span> </a></li><span class='dropdown-toggle option-icon' title='" + window.Server.App.LocalizationContent.Action + "' data-toggle='dropdown'><span class='su su-options-horizontal'></span></span><ul class='dropdown-menu' role='menu'><li class='edit' title='" + window.Server.App.LocalizationContent.EditButton + "'  data-toggle='tooltip' data-placement='top'> <a class='edit-comment'><span>" + window.Server.App.LocalizationContent.EditButton + "</span> </a></li><li class='delete' title='" + window.Server.App.LocalizationContent.Delete + "'  data-toggle='tooltip' data-placement='top'> <a class='delete-comment'><span>" + window.Server.App.LocalizationContent.Delete + "</span> </a></li></menu><span class='post-meta col-xs-9 no-padding'><a href='#comment-"
    ...
    $(document).on('click', '.edit-comment', function () {
        ...
        $("#save-edit").on('click', function () {
            ...
            var commentHTML = editMd.options.previewRender(comment).replace("#", "");
            var commentOriginalValue = comment;
            var commentTextValue = commentOriginalValue.replace("http://", '')
                                                       .replace("https://", '')
                                                       .replace("(", '')
            ...
            if (comment.length > 4000) {
                ...
                parent.ajaxPostCall("POST", editCommentUrl, values, null, OnCommentEdited, null, null);
            }
        });
    ...
    $(document).on('click', '.reply-comment', function () {
        ...
        $("#post-reply").on('click', function () {
            ...
            var comment = replyMd.value();
            var textAreaElementNode = $('#replyComment > .CodeMirror');
            var userMention = splitUserMention(textAreaElementNode);
            comment = getRelativePath(comment);
            var parentId = $(this).closest('.post').attr('data-comment-id');
            var values = { itemId: itemId, comment: comment, parentId: parentId, itemType: itemType, dashboardItemId: dashboardItemId, url: decodeURI(currentPageUrl), uniqueUsersValue: userMention.uniqueUsersValue, uniqueGroupValue: userMention.uniqueGroupValue, mentionedAll: userMention.mentionedAll, parentDashboardId: parentDashboardId, currentDate: date };
            var addComment = "";
            var commentHTML = replyMd.options.previewRender(comment).replace("#", "");
            var commentOriginalValue = comment;
            var commentTextValue = commentOriginalValue.replace("http://", '')
                                                       .replace("https://", '')
                                                       .replace("(", '')
            ...
            if (comment.length > 4000) {
                ...
                parent.ajaxPostCall("POST", addCommentUrl, values, null, OnReplyPosted, null, null);
            }
        });
    ...
    function OnReplyPosted(data) {
        if (data) {
            ...
            addComment += "<li class='reply' title='" + window.Server.App.LocalizationContent.ReplyButton + "'data-toggle='tooltip' data-placement='bottom'> <a class='reply-comment'><span class='su su-reply'></span> </a></li><span class='dropdown-toggle option-icon' title='" + window.Server.App.LocalizationContent.Action + "' data-toggle='dropdown'><span class='su su-options-horizontal'></span></span><ul class='dropdown-menu' role='menu'><li class='edit' title='" + window.Server.App.LocalizationContent.EditButton + "'  data-toggle='tooltip' data-placement='top'> <a class='edit-comment'><span>" + window.Server.App.LocalizationContent.EditButton + "</span> </a></li><li class='delete' title='" + window.Server.App.LocalizationContent.Delete + "'  data-toggle='tooltip' data-placement='top'> <a class='delete-comment'><span>" + window.Server.App.LocalizationContent.Delete + "</span> </a></li><li class='parent' data-parent-id='"
                ...
                + $('#replyComment').parents('.post').find('.author').attr('title') + "'> <a class='parent-name'><span>" + window.Server.App.LocalizationContent.ShowParent + "</span> </a></li></ul></menu><span class='post-meta col-xs-9 no-padding'><a href='#comment-"
                + data.Comments[0].Id + "'class='permalink'><span class='time-ago' title='"
                + data.Comments[0].CreatedDateString + " - " + data.Comments[0].TimeZoneName + "'data-html='false' data-toggle='tooltip' data-placement='right'>"
                + data.Comments[0].TimeAgo + "</span></a></span> </header> <div class='post-body-inner'><div class='edit-post displayNone'></div><div class='post-message'>"
                + data.Comments[0].HtmlComment + "</div><div class='markDownComment hidden'>"
            ...
            if (parseInt($("#replyComment").parent().find('.count-text').text()) == 1) {
                $("#replyComment").parent().find('.count-label').text(window.Server.App.LocalizationContent.ReplyMessage);
            } else {
                $("#replyComment").parent().find('.count-label').text(" " + window.Server.App.LocalizationContent.Replies);
            }
    ...
    $(document).on('click', '.delete-comment', function () {
        ...
        parent.messageBox("su-delete", window.Server.App.LocalizationContent.DeleteComment, window.Server.App.LocalizationContent.DeleteComentConfirm, "error", function () {
            ...
            var itemRequest = { itemId: itemId, commentId: commentId, itemType: itemType, dashboardItemId: dashboardItemId, url: decodeURI(currentPageUrl), commentAction: commentAction.Delete};
    ...
    function OnCommentDeleted(data) {
        if (data) {
            ...
            parent.messageBox("su-delete", window.Server.App.LocalizationContent.DeleteComment, window.Server.App.LocalizationContent.DeleteCommnetSuccess, "success", function () {
                ...
                if (replyCount == 2) {
                    $("#" + postId).parents('.post:first').find('.count-label:first').html(window.Server.App.LocalizationContent.ReplyMessage);
                }
        ...
        window.top.$("#messageBox_wrapper").ejWaitingPopup("hide");
    }
...
function GetAllComments(itemId, itemType, dashboardItemId, orderBy) {
    var skip = 0;
    var take = 0;
    var itemRequest = { orderBy: orderBy, skip: skip, take: take, itemType: itemType, itemId: itemId, dashboardItemId: dashboardItemId, commentAction: commentAction.GetComments, parentDashboardId: parentDashboardId };
    showWaitingPopup("parent-scroll-element");
        ...
        for (var i = 0; i < item.length; i++) {
            ...
            if (!item[i].IsActive) {
                commentHtml += "<li id='comment-"
                    ...
                    + item[i].Id + "' class='post'><div class='post-content'><div class='avatar-container'><img src='" + avatarUrl + "' class='avatar'></div><div class='post-body'> <header><span class='post-byline col-xs-5 no-padding'></span><span class='post-meta col-xs-9 hidden no-padding'><a href='#comment-"
                    ...
                    + childCount + "</span><span class='count-label'>" + (childCount != 1 ? window.Server.App.LocalizationContent.Replies : window.Server.App.LocalizationContent.ReplyMessage) + "</span></div><div class='col-xs-6 no-padding'>";
                if (childCount > 0) {
                ...
                commentHtml += "<li id='comment-"
                    ...
                    + item[i].UserId + "'><img src='" + avatarUrl + "?id="
                    + item[i].IdPReferenceId + "&ImageSize=32' class='avatar'> </div></div><div class='post-body'> <header><span class='post-byline col-xs-5 no-padding'><span title='"
                ...
                commentHtml += "</menu><span class='post-meta col-xs-9 no-padding'><a href='#comment-"
                    ...
                    + childCount + "</span><span class='count-label'>" + (childCount != 1 ? window.Server.App.LocalizationContent.Replies : window.Server.App.LocalizationContent.ReplyMessage) + "</span></div><div class='col-xs-6 no-padding collapse-icon-container'><span class='su ";
                if (childCount > 0) {
...
function truncateUrl() {
    ...
    var isDashboardExist = currentPageUrl.substr(0, currentPageUrl.indexOf('/reports') + 1);
...
function pasteImageReader(dataURL) {
    ...
    var values = { DataUrl: dataUrl, itemId: itemId, itemType: itemType, parentDashboardId: parentDashboardId };
    parent.ajaxPostCall("POST", commentImageUrl, values, null, pasteCommentImages, null, null);
}
...
function autosuggestion(keyword) {
    $.xhrPool.abortAll();
    var itemId = parent.$("#dashboard_Comment").attr("data-item-id");
...
$(document).on("click", "#close-comment", function () {
    ...
    if (iframeUrl == parentUrl) {
        parent.$(dashboardTypeId).css("width", $(parent.window).width());
    }
    else {
